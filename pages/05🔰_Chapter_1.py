...
def _exists(url: str) -> bool:
    """Check if a GitHub raw URL exists."""
    try:
...
def discover_pngs_by_pattern(raw_base: str, prefix: str, ext: str, start_i: int, end_i: int):
    """Probe sequentially named PNGs like F25_Ch01.001.png, F25_Ch01.002.png ..."""
    found = []
    for i in range(start_i, end_i + 1):
        name = f"{prefix}{i:03d}{ext}"   # zero-padded, e.g., 001, 002, 003
        url  = f"{raw_base}/{name}"
        if _exists(url):
...
if not slides:
    st.error(
        ...
        "Please check the folder path, file names, and END_INDEX."
    )
    st.stop()
...
if "pending_jump" not in st.session_state:
    st.session_state.pending_jump = False  # flag for thumbnails

def clamp(i: int) -> int:
    return max(0, min(len(slides) - 1, i))

# Sync jump_num before rendering widget
...
with st.sidebar:
    ...
    if not fit_to_width:
        st.session_state["display_width"] = st.slider(
            "Slide width (px)", 700, 1100, DISPLAY_WIDTH_DEFAULT, step=50
        )

# ===== Main area: slide + caption =====
idx = st.session_state.slide_idx
if st.session_state.get("fit_to_width", True):
    st.image(slides[idx], use_column_width=True, caption=f"Slide {idx + 1} / {len(slides)}")
else:
    st.image(
        slides[idx],
        width=st.session_state.get("display_width", DISPLAY_WIDTH_DEFAULT),
